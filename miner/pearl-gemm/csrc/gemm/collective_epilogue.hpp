...
struct CollectiveEpilogue {
  ...
  CUTLASS_DEVICE void denoise(FrgTensor& tCrD,  // fp32
    ...
    // Blackwell (sm_120/121): SM80 mma.sync needs explicit ldmatrix
    // staging from SMEM into registers.  On Hopper these fragments are
    // WGMMA descriptors that read SMEM directly, so the staging below is
    // compiled out.
    ...
    auto s2r_tiled_copy_A_denoise =
        make_tiled_copy_A(S2RCopyAtomDenoise_A{}, tiled_mma_denoise);
    auto s2r_tiled_copy_B_denoise =
        make_tiled_copy_B(S2RCopyAtomDenoise_B{}, tiled_mma_denoise);
    ...
    Tensor tXsAxEBL_s2r =
        s2r_thr_copy_A_denoise.partition_S(sAxEBL_mma_no_pi(_, _, _0{}));
    Tensor tXsEBR_s2r =
        s2r_thr_copy_B_denoise.partition_S(sEBR_mma_no_pi(_, _, _0{}));
    Tensor tYsEAL_s2r =
        s2r_thr_copy_A_denoise.partition_S(sEAL_mma_no_pi(_, _, _0{}));
    Tensor tYsEARxBpEB_s2r =
        s2r_thr_copy_B_denoise.partition_S(sEARxBpEB_mma_no_pi(_, _, _0{}));
